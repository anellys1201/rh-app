...
export default function AgregarEmpleado() 
{
    ...
    useEffect(() => {
        axios.get("/rh-app/bienes")
            .then(res=>{
                setCatalogoBienes(res.data);
            });
    }, []);
    ...
    const onSubmit = async (e) => {
        ...
        if(!archivo){
            Swal.fire({
                title: 'Advertencia',
                text: 'Falta agregar un documento',
                icon: 'warning',
        ...
        try {
            ...
            Swal.fire({
                title: 'Correcto',
                text: 'Empleado y documento guardados',
                icon: 'success',
                confirmButtonText: 'Aceptar'
            });
    ...
    /*const onBienChange = (index, e) => {
        ...
        setEmpleado({...empleado, bienes:nuevosBienes});
            ...
            <button type="button" className='btn btn-primary btn-sm mb-3' onClick={agregarBien}>Agregar Bien</button>
            {
                empleado.bienes.map((item, index) => (
                    ...
                    <div key={index} className='border p-2 mb-2'>
                        <select className='form-control mb-2' name='idBien' onChange={(e)=>onBienChange(index,e)}>
                            <option value="">Seleccione bien</option>
                            {
                                catalogoBienes.map(bien=>(
                                    <option key={bien.idBien} value={bien.idBien}>{bien.nombre}</option>
                        ...
                        <input type='text' className='form-control' placeholder='Comentario' name='comentario' onChange={(e)=>onBienChange(index,e)}/>
